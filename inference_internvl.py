...
path = '/mnt/checkpoints/open_source_debug/stage2_internvl/checkpoint-100'
model = InternVLWithHiMTok.from_pretrained(
    path,
...
pixel_values = [load_image(image, max_num=4).to(torch.bfloat16).cuda() for image in images]
num_patches_list = [pixel_values[i].size(0) for i in range(len(pixel_values))]
pixel_values = torch.cat(pixel_values, dim=0)

questions = ['<image>\nSegment <ref>animal</ref>.'] * len(image_paths)
responses, masks = model.batch_chat(tokenizer, pixel_values,
