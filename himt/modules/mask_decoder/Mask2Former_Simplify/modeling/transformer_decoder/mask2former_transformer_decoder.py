...
class MultiScaleMaskedTransformerDecoderForOPTPreTrain(nn.Module):
    ...
    def forward(self, x, mask_features, mask=None, seg_query=None, SEG_embedding=None, class_name_embedding=None, region_embedding_list=None, using_box=False, using_boxiou = False, return_hs=False):
        if self.seg_concat: # False
            return self.forward_concat(x, mask_features, mask, seg_query, SEG_embedding, class_name_embedding, region_embedding_list)
        else:
            return self.forward_woconcat(x, mask_features, mask, seg_query, SEG_embedding, class_name_embedding, region_embedding_list, using_box, using_boxiou, return_hs)
    ...
    def forward_woconcat(self, x, mask_features, mask=None, seg_query=None, SEG_embedding=None,
                         class_name_embedding=None, region_embedding_list=None, using_box=False, using_boxiou=False, return_hs=False):
        # x is a list of multi-scale feature
        assert len(x) == self.num_feature_levels
        src = []
        pos = []
        ...
        # prediction heads on learnable query features
        SEG_class, class_name_class, outputs_mask, outputs_box, outputs_boxiou, attn_mask, region_class_list = self.forward_prediction_heads(output,
                                                                                                                mask_features,
                                                                                                                attn_mask_target_size=
                                                                                                                size_list[
                                                                                                                    0],
                                                                                                                SEG_embedding=SEG_embedding,
                                                                                                                class_name_embedding=class_name_embedding,
                                                                                                                region_embedding_list=region_embedding_list, using_box=using_box, using_boxiou=using_boxiou)
        ...
        predictions_boxiou.append(outputs_boxiou)
        hs_feature = None
        ...
        for i in range(self.num_layers):
            ...
            if return_hs and i == self.num_layers-1:
                hs_feature = output.transpose(0, 1)

            SEG_class, class_name_class, outputs_mask, outputs_box, outputs_boxiou, attn_mask, region_class_list = self.forward_prediction_heads(
                output, mask_features,
                attn_mask_target_size=
                size_list[(
                                  i + 1) % self.num_feature_levels],
        ...
        out = {
            ...
            'pred_boxious': predictions_boxiou[-1],
            'hs': hs_feature,
            'aux_outputs': self._set_aux_loss(
                predictions_SEG_class, predictions_class_name_class, predictions_mask, predictions_box, predictions_boxiou, predictions_region_class
            )
        }
        return out

    def forward_prediction_heads(self, output, mask_features, attn_mask_target_size, SEG_embedding=None,
                                 class_name_embedding=None, region_embedding_list=None, using_box=False, using_boxiou=False):
        decoder_output = self.decoder_norm(output)
        decoder_output = decoder_output.transpose(0, 1)
        ...
        # boxiou pred
        if using_boxiou:
            outputs_boxiou = self.iou_head(decoder_output)
        else:
            outputs_boxiou = None
        ...
        return SEG_class, class_name_class, outputs_mask, outputs_box, outputs_boxiou, attn_mask, region_class_list

    @torch.jit.unused
    def _set_aux_loss(self, outputs_SEG_class, outputs_class_name_class, outputs_seg_masks, outputs_seg_boxes, outputs_seg_boxiou, predictions_region_class):
        # this is a workaround to make torchscript happy, as torchscript
        # doesn't support dictionary with non-homogeneous values, such
        # as a dict having both a Tensor and a list.
        # if self.mask_classification:
        ...
        return [
            {"pred_SEG_logits": a, "pred_class_name_logits": b, "pred_masks": c, "pred_region_logits": d, "pred_boxes": e, "pred_boxious": f}
            for a, b, c, d, e, f in zip(outputs_SEG_class[:-1], outputs_class_name_class[:-1], outputs_seg_masks[:-1],
                                  predictions_region_class[:-1], outputs_seg_boxes[:-1], outputs_seg_boxiou[:-1])
        ]
