...
class TiTokDecoder(nn.Module):
    ...
    def patchify(self, x):
        ...
        x = x.reshape(x.shape[0], x.shape[1], -1)
        x = x.permute(0, 2, 1)  # L, B, C
        return x
    
    def forward(self, z_quantized, image_src=None, pixel_decoder=None,random_length=True):
        N, C, H, W = z_quantized.shape
        assert H == 1 and W <= self.num_latent_tokens, f"{H}, {W}, {self.num_latent_tokens}"
        x = z_quantized.reshape(N, C*H, W).permute(0, 2, 1)  # NLD
        x = self.decoder_embed(x)

        if self.training and random_length:
        ...
        src_tokens = self.patchify(image_src)  #BxTxD
        src_tokens = src_tokens + self.positional_embedding[1:self.grid_size**2+1].to(src_tokens.dtype)
        src_tokens = src_tokens.permute(1, 0, 2)  # T, B, D

        # Forward through transformer
        x = self.ln_pre(x)
        x = x.permute(1, 0, 2)  # NLD -> LND
        for i in range(self.num_layers):
            x = self.transformer[i](x)  # self attention
            x = self.cross_attention[i](x, src_tokens)  # cross attention with source tokens
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, context):
        ...
        # context: B, S, D -> S, B, D
        L, B, D = x.shape
        S = context.shape[0]  # source sequence length

        # Project q from x, and k,v from context
        q = self.q(x)  # L, B, D
        k = self.k(context)  # S, B, D
        v = self.v(context)  # S, B, D

        if ATTENTION_MODE == 'flash':
            q = einops.rearrange(q, 'L B (H D) -> B H L D', H=self.num_heads).float()
            k = einops.rearrange(k, 'S B (H D) -> B H S D', H=self.num_heads).float()
            v = einops.rearrange(v, 'S B (H D) -> B H S D', H=self.num_heads).float()
            x = torch.nn.functional.scaled_dot_product_attention(q, k, v)
            x = einops.rearrange(x, 'B H L D -> L B (H D)')
        elif ATTENTION_MODE == 'xformers':
            q = einops.rearrange(q, 'L B (H D) -> B L H D', H=self.num_heads)
            k = einops.rearrange(k, 'S B (H D) -> B S H D', H=self.num_heads)
            v = einops.rearrange(v, 'S B (H D) -> B S H D', H=self.num_heads)
            x = xformers.ops.memory_efficient_attention(q, k, v)
            x = einops.rearrange(x, 'B L H D -> L B (H D)', H=self.num_heads)
        else:
            q = einops.rearrange(q, 'L B (H D) -> B H L D', H=self.num_heads)
...
class CrossAttentionBlock(nn.Module):
    ...
    def forward(self, x, context):
        x = x + self.cross_attn(self.ln_1(x), self.ln_2(context))
        return x